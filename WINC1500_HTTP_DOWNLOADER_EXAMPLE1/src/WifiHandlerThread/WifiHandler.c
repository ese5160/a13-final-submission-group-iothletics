...
QueueHandle_t xQueueImuBuffer = NULL;       ///< Queue to send IMU X data to the cloud
QueueHandle_t xQueueDistanceBuffer = NULL;  ///< Queue to send the distance to the cloud
QueueHandle_t xQueueLoadBuffer = NULL;  ///< Queue to send the Load Cell data to the cloud
...
static void MQTT_HandleLoadMessages(void);
//static void MQTT_HandleHapticMessages(void);
...
/* void SubscribeHandlerHapticTopic(MessageData *msgData)
{
	LogMessage(LOG_DEBUG_LVL, "\r\nHaptic topic received!\r\n");
	LogMessage(LOG_DEBUG_LVL, "\r\n %.*s", msgData->topicName->lenstring.len, msgData->topicName->lenstring.data); 
}*/
    ...
    switch (type) {
        ...
        case MQTT_CALLBACK_CONNECTED:
            if (data->connected.result == MQTT_CONN_RESULT_ACCEPT) {
				...
				//mqtt_subscribe(module_inst, HAPTIC_TOPIC, 2, SubscribeHandlerHapticTopic);
//                 mqtt_subscribe(module_inst, IMUX_TOPIC, 2, SubscribeHandlerImuXTopic);
// 				mqtt_subscribe(module_inst, IMUY_TOPIC, 2, SubscribeHandlerImuYTopic);
// 				mqtt_subscribe(module_inst, IMUZ_TOPIC, 2, SubscribeHandlerImuZTopic);
// 				mqtt_subscribe(module_inst, LOAD_TOPIC, 2, SubscribeHandlerLoadTopic);
...
static void MQTT_HandleTransactions(void)
{
	...
	/* MQTT_HandleImuYMessages();
	MQTT_HandleImuZMessages(); */
	MQTT_HandleLoadMessages();
...
static void MQTT_HandleImuMessages(void)
{
    ...
    if (pdPASS == xQueueReceive(xQueueImuBuffer, &imuPacket, 0)) {
        snprintf(mqtt_msg, 63, "%d", imuPacket.xmg);
        mqtt_publish(&mqtt_inst, IMUX_TOPIC, mqtt_msg, strlen(mqtt_msg), 1, 0);
		snprintf(mqtt_msg, 63, "%d", imuPacket.ymg);
		mqtt_publish(&mqtt_inst, IMUY_TOPIC, mqtt_msg, strlen(mqtt_msg), 1, 0);
	    snprintf(mqtt_msg, 63, "%d", imuPacket.zmg);
		mqtt_publish(&mqtt_inst, IMUZ_TOPIC, mqtt_msg, strlen(mqtt_msg), 1, 0);
    }
}
...
{
	int haptic;
	if (pdPASS == xQueueReceive(xQueueHapticBuffer, &haptic, 0)) {
		snprintf(mqtt_msg, 63, "{\"Haptic\":%d}", haptic);
		mqtt_publish(&mqtt_inst, HAPTIC_TOPIC, mqtt_msg, strlen(mqtt_msg), 1, 0);
	}
} */
...
void vWifiTask(void *pvParameters)
{
	...
	//xQueueImuYBuffer = xQueueCreate(5, sizeof(struct ImuDataPacket));
	//xQueueImuZBuffer = xQueueCreate(5, sizeof(struct ImuDataPacket));
    xQueueGameBuffer = xQueueCreate(2, sizeof(struct GameDataPacket));
    xQueueDistanceBuffer = xQueueCreate(5, sizeof(uint16_t));
	xQueueLoadBuffer = xQueueCreate(5, sizeof(uint32_t));

    if (xQueueWifiState == NULL || xQueueImuBuffer == NULL /*|| xQueueImuYBuffer == NULL || xQueueImuZBuffer == NULL*/ || 
		xQueueLoadBuffer == NULL || xQueueGameBuffer == NULL || xQueueDistanceBuffer == NULL) {
...
/* int WifiAddImuYDataToQueue(struct ImuDataPacket *imuPacket)
{
	int error = xQueueSend(xQueueImuYBuffer, imuPacket->ymg, (TickType_t)10);
	return error;
}

int WifiAddImuZDataToQueue(struct ImuDataPacket *imuPacket)
{
	int error = xQueueSend(xQueueImuZBuffer, imuPacket->zmg, (TickType_t)10);
	return error;
} */

int WifiAddLoadDataToQueue(uint32_t *pressure)
{
	int error = xQueueSend(xQueueLoadBuffer, pressure, (TickType_t)10);
	return error;
}

/* int WifiAddHapticDataToQueue(int *error)
{
	int haptic_error = xQueueSend(xQueueHapticBuffer, error, (TickType_t)10);
	return haptic_error;
} */
